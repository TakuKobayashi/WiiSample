...
3. If the connection succeeds, the remote will automatically set its LEDs accordingly and you will be able to access its input through the API. A message will also be broadcast by the Wii prefab if it is in the scene and the Wii class's delegate events will fire.
...
- During tests the connection rate has been fairly reliable. I plan on making it more so with future updates. Some remotes still drop after a few seconds while using OSX11 (El Capitán) but I've implemented a unique connection function for that OSX which has greatly reduced this (As a result though the connection involves pairing the remote each time. So pushing random buttons on the controller later may cause dumb pop ups to appear. Sorry.).
...
HOW TO CONNECT WIIMOTES (on Windows)

1. Do not have Unity open.

2. Open Start > Control Panel > Devices and Printers.

3. If any of your wiimotes are listed under devices, remove them.

4. Select "Add Device".

5. A list of all the bluetooth devices your computer sees available will appear. While this is happening, press the red sync button.

6. As soon as your remote or balance board appears on the list of available devices. Double click on it and select "Pair without using a code" before the lights on your remote stop blinking.
	-If the remote is a newer one with motion plus built in and is labeled "Nintendo RVL-CNT-01-TR", I'm sorry but WiiBuddy for Windows does not yet support this device.
	
7. Your controller now has a base-connection with your computer.
	-The process listed above is the same that any Windows Wiimote program uses.
	
8. Open your Unity Project containing WiiBuddy.

9. Begin the search by calling the StartSearch() function (the Wii prefab's inspector comes with a 'Find' button at the top.).

10. If the connection succeeds, the remote will automatically set its LEDs accordingly and you will be able to access its input through the API. A message will also be broadcast by the Wii prefab if it is in the scene and the Wii class's delegate events will fire.

- If no remote is found is found before the search ends, the discoveryStatus will become -100 and that discovery error will be broadcast.

- If a remote is found but the connection fails, the discovery status will become -536870195 and that discovery error will be broadcast. This happens from time to time and doesn't necessarily mean you did anything wrong. So just try again. If the problem persists, open up your computer's bluetooth preferences and remove the remote from the list of familiar devices. This pretty much results in a 100% connection rate.

- If a remote is found but it is an unsupported "Nintendo RVL-CNT-01-TR" remote as mentioned above, the discovery error will be -816. 

11. When you're done with the remote, call DropWiiRemote(int) to disconnect it from your computer. Whilst using the editor, your remotes will stay connected regardless of whether you start or stop playing the current scene. When closing the standalone app of your project, all remotes will automatically disconnect.
...
- Wii® Remote Plus ("Nintendo RVL-CNT-01-TR")(Mac only)
- Motion Plus
- Nunchuck
...
JavaScript
	...
	4. Wii.cs will also trigger delegate events to the same effect as the broadcast messages. You can add listeners for these in your own scripts.
...
C#
	...
	2. Wii.cs will also trigger delegate events for remote discovery. You can add listeners for these in your own scripts.

	3. If you want to receive the Wii class's broadcast message, you'll still need to place the "Wii" prefab into your scene as its children will be the ones receiving the messages.
